...
export function createScene(container,model,data) {
  ...
  const grid=new THREE.GridHelper(20,200,0x476274,0x2e4454);grid.rotation.x=Math.PI/2;grid.position.z=.00015;grid.material.transparent=true;grid.material.opacity=.5;scene.add(grid);
  ...
  for(let i=0;i<model.ngeom;i++) {
    ...
    const mesh=new THREE.Mesh(geometry,new THREE.MeshStandardMaterial({color:new THREE.Color(rgba[0],rgba[1],rgba[2]),roughness:.65,metalness:.05,flatShading:true}));
    mesh.castShadow=true;mesh.receiveShadow=true;mesh.matrixAutoUpdate=false;scene.add(mesh);meshes.push({mesh,id:i});
  }
  ...
  return {count:meshes.length,draw(follow=true){
